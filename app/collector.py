    ...
    try:
        ...
        try:
            ...
            for a in alerts:
                conn.execute(
                    """INSERT INTO alerts
                       (alert_id, first_seen, last_seen, route_ids, stop_ids, header, description,
                        effect, cause, valid_from, valid_until, active)
                       VALUES (:alert_id, :now, :now, :route_ids, :stop_ids, :header, :description,
                               :effect, :cause, :valid_from, :valid_until, 1)
                       ON CONFLICT(alert_id) DO UPDATE SET
                           last_seen=:now, route_ids=:route_ids, stop_ids=:stop_ids, header=:header,
...
def check_cancellation_alerts_job():
    ...
    try:
        ...
        for operator, a in per_operator.items():
            ...
            description = (
                f"Bij {operator} is vandaag {pct:.1f}% van de busritten uitgevallen. "
                "Klik hier voor meer. Let op: data kunnen licht afwijken van precieze "
                "cijfers. Gebruik bij evt. berichtgeving geen exacte percentages."
            )
            conn.execute(
                """INSERT OR IGNORE INTO rss_feed_items
                   (guid, kind, title, description, pub_date, created_at)
